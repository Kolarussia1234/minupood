...
class ProductController extends ProductControllerCore
{
    ...
    protected function assignAttributesGroups($product_for_template = null)
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            $attributeIdProductMap = [];
            foreach ($attributes_groups as $k => $row) {
                // Color management

                if (isset($row['is_color_group']) && $row['is_color_group'] && (isset($row['attribute_color']) && $row['attribute_color']) || (file_exists(_PS_COL_IMG_DIR_.$row['id_attribute'].'.jpg'))) {
                    if(!isset($attributeIdProductMap[(int) $row['id_attribute']])){
                        $id = Combination::getImageByIdProductAttributeANDIdAttribute((int)$row['id_attribute'],(int)$this->product->id);
                        $path = _PS_PROD_IMG_DIR_SHORT_;
                        $path2 = $path;
                        for ($i=0; isset($id[$i]); $i++) {
                            $path2 = $path2 . $id[$i]. '/' ;
                        }
                        $path2 = $path2  . $id .'.jpg';
                        $attributeIdProductMap[(int) $row['id_attribute']] = $path2;
}
                ...
                $texture = '';
                if(isset($attributeIdProductMap[(int) $row['id_attribute']])){
                    $texture = $attributeIdProductMap[(int) $row['id_attribute']];
                } else if (@filemtime(_PS_COL_IMG_DIR_.$row['id_attribute'].'.jpg')){
            ...
            foreach ($groups as &$group) {
                ...
                if ($count > 1) {
                    ...
                    $query = 'SELECT ps_product_attribute_combination.`id_product_attribute`
                              ...
                              WHERE ps_product_attribute.id_product = '.$this->product->id.' 
