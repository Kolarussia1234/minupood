...
<input type="hidden" name="vp_smartpost_{$id_address|escape:'htmlall':'UTF-8'}_ajax" value="{$ajax_url|escape:'html':'UTF-8'}" class="vp_smartpost_{$id_address|escape:'html':'UTF-8'}_ajax"/>
{assign var="group_id" value='-1'}
<select name='terminals' class='veebipoed_carrier' id='smartpost_{$id_address|escape:'html':'UTF-8'}' onchange="updateCarrierTerminal({$id_address|escape:'html':'UTF-8'}, '{$carrier.name|escape:'html':'UTF-8'}', 'vp_smartpost');">
	...
	{foreach from=$terminals item=terminal}
		...
		<option value='{$terminal.id_vp_smartpost|escape:'html':'UTF-8'}'{if $terminal.id_vp_smartpost == $carrier.terminal_id}selected="selected"{/if}
