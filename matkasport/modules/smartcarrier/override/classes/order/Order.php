...
class Order extends OrderCore
{
	public function getShipping()
	{
		...
		$shipping =  Db::getInstance()->executeS('
			SELECT DISTINCT o.id_cart, oc.`id_order_invoice`, oc.`weight`, oc.`shipping_cost_tax_excl`, oc.`shipping_cost_tax_incl`, c.`url`, oc.`id_carrier`, c.`name` as `carrier_name`, c.external_module_name, oc.`date_add`, "Delivery" as `type`, "true" as `can_edit`, oc.`tracking_number`, oc.`id_order_carrier`, osl.`name` as order_state_name, c.`name` as state_name
