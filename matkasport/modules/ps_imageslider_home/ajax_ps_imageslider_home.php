...
include_once('ps_imageslider_home.php');

$home_slider = new Ps_ImageSlider_Home();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'homebanner_slides` SET `position` = '.(int)$position.'
			WHERE `id_homebanner_slides` = '.(int)$id_slide
		);
