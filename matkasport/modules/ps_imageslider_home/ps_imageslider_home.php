...
include_once(_PS_MODULE_DIR_.'ps_imageslider_home/Ps_HomeSlide_Home.php');

class Ps_ImageSlider_Home extends Module implements WidgetInterface
{
    const DESC_TOP = 0;
    const DESC_MID = 1;
    const DESC_BOT = 2;
    ...
    const TOP_BANNER = 0;
    const SM_TOP_LEFT_BANNER = 1;
    const SM_TOP_RIGHT_BANNER = 2;
    const SM_BOTTOM_LEFT_BANNER = 3;
    const SM_BOTTOM_RIGHT_BANNER = 4;
    ...
    {
        $this->name = 'ps_imageslider_home';
        $this->tab = 'front_office_features';
        $this->version = '3.0.0';
        $this->author = 'PrestaShop (Modified by Veebipoed.ee)';
        ...
        $this->displayName = $this->getTranslator()->trans('Homepage banners', array(), 'Modules.Imageslider.Admin');
        $this->description = $this->getTranslator()->trans('Adds either banners/sliders to your homepage.', array(), 'Modules.Imageslider.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.4.0', 'max' => _PS_VERSION_);

        for ($i=0; $i <= count($this->getBannerHooks()) - 1; $i++)
            $this->templateFiles[] = 'module:ps_imageslider_home/views/templates/hook/slider'.$i.'.tpl';
    }
    ...
    public function install()
    {
        /* Adds Module */
        if (parent::install() &&
            $this->registerHook(['header', 'actionShopDataDuplication']) && 
            $this->registerHook($this->getBannerHooks())
        ) {
    ...
    private function getBannerHooks()
    {
        return array('displayTopMainBanner',
            'displaySmallBannerTopLeft',
            'displaySmallBannerTopRight',
            'displaySmallBannerBottomLeft',
            'displaySmallBannerBottomRight');
    }
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            $slide = new Ps_HomeSlide_Home();
            $slide->position = $i;
            $slide->active = 1;
            $slide->desc_pos = self::DESC_MID;
            $slide->slot = self::TOP_BANNER;
            foreach ($languages as $language) {
                ...
                $slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=back-office&utm_medium=v17_homebanner'
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'homebanner` (
                `id_homebanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_homebanner_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'homebanner_slides` (
              `id_homebanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              `desc_pos` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              `slot` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_homebanner_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'homebanner_slides_lang` (
              `id_homebanner_slides` int(10) unsigned NOT NULL,
              ...
              PRIMARY KEY (`id_homebanner_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new Ps_HomeSlide_Home($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'homebanner`, `'._DB_PREFIX_.'homebanner_slides`, `'._DB_PREFIX_.'homebanner_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($this->_postValidation()) {
                ...
                foreach ($this->getBannerHooks() as $hook) {
                    $this->_html .= $this->renderList($this->getSlotByHookName($hook));
                }
            } else {
            ...
            $this->clearCache();
        } elseif (Tools::isSubmit('addSlide') && Tools::isSubmit('slot') || (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))) {
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = Ps_HomeSlide_Home::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            /* Checks description position */
            if (!Validate::isInt(Tools::getValue('desc_pos')) || !in_array(Tools::getValue('desc_pos'), [self::DESC_TOP,self::DESC_MID,self::DESC_BOT])) {
                $errors[] = $this->getTranslator()->trans('Invalid description position on slide.', array(), 'Modules.Imageslider.Admin');
            }
            /* Checks description position */
            if (!Validate::isInt(Tools::getValue('slot')) || !in_array(Tools::getValue('slot'), [self::TOP_BANNER,self::SM_TOP_RIGHT_BANNER,self::SM_BOTTOM_RIGHT_BANNER,self::SM_TOP_LEFT_BANNER,self::SM_BOTTOM_LEFT_BANNER])) {
                $errors[] = $this->getTranslator()->trans('Invalid slot id.', array(), 'Modules.Imageslider.Admin');
            }
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            $slide = new Ps_HomeSlide_Home((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            /* Sets description position */
            $slide->desc_pos = (int)Tools::getValue('desc_pos');
            /* Sets slot */
            $slide->slot = (int)Tools::getValue('slot');
            ...
            $slide = new Ps_HomeSlide_Home((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    {
        $this->context->controller->addCSS($this->_path.'/views/css/homebanner.css');
        $this->context->controller->addJS([$this->_path.'/views/js/responsiveslideshome.min.js',$this->_path.'/views/js/homebanner.js']);
    }
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        $slot = (int)$this->getSlotByHookName($hookName);
        
        if (!$this->isCached($this->templateFiles[$slot], $this->getCacheId())) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFiles[$slot], $this->getCacheId());
    }

    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        $slides = $this->getSlides(true, $this->getSlotByHookName($hookName));
        if (is_array($slides)) {
            foreach ($slides as &$slide) {
    ...
    public function clearCache()
    {
        foreach ($this->templateFiles as $templateFile) {
            ...
            Module::getInstanceByName('iqitelementor')->clearHomeCache();
        }
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'homebanner (id_homebanner_slides, id_shop)
            SELECT id_homebanner_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'homebanner
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'homebanner_slides` hss, `'._DB_PREFIX_.'homebanner` hs
            WHERE hss.`id_homebanner_slides` = hs.`id_homebanner_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );

        return (++$row['next_position']);
    }

    public function getSlotByHookName($name)
    {
        switch ($name) {
            case 'displaySmallBannerTopRight':
                return self::SM_TOP_RIGHT_BANNER;

            case 'displaySmallBannerTopLeft':
                return self::SM_TOP_LEFT_BANNER;

            case 'displaySmallBannerBottomRight':
                return self::SM_BOTTOM_RIGHT_BANNER;

            case 'displaySmallBannerBottomLeft':
                return self::SM_BOTTOM_LEFT_BANNER;

            case 'displayTopMainBanner':
    ...
    public function getSlotNameById($id)
    {
        switch ((int)$id) {
            case self::SM_TOP_RIGHT_BANNER:
                return $this->getTranslator()->trans('Top right banner', array(), 'Modules.Imageslider.Admin');

            case self::SM_TOP_LEFT_BANNER:
                return $this->getTranslator()->trans('Top left banner', array(), 'Modules.Imageslider.Admin');

            case self::SM_BOTTOM_RIGHT_BANNER:
                return $this->getTranslator()->trans('Bottom right banner', array(), 'Modules.Imageslider.Admin');

            case self::SM_BOTTOM_LEFT_BANNER:
                return $this->getTranslator()->trans('Bottom left banner', array(), 'Modules.Imageslider.Admin');

            case self::TOP_BANNER:
                return $this->getTranslator()->trans('Main top banner', array(), 'Modules.Imageslider.Admin');
    ...
    public function getDescPosName($i = null) {
        switch ($i) {
            case 0:
                return 'top';
    ...
    public function getSlides($active = null, $slot = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_homebanner_slides` as id_slide, hss.`position`, hss.`active`, hss.`desc_pos`, hss.`slot`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'homebanner hs
            LEFT JOIN '._DB_PREFIX_.'homebanner_slides hss ON (hs.id_homebanner_slides = hss.id_homebanner_slides)
            LEFT JOIN '._DB_PREFIX_.'homebanner_slides_lang hssl ON (hss.id_homebanner_slides = hssl.id_homebanner_slides)
            WHERE id_shop = '.(int)$id_shop.'
            AND hssl.id_lang = '.(int)$id_lang.
            (Validate::isInt($slot) && $slot>=0 && $slot<=4 ? ' AND hss.`slot` = '.$slot : ' ').
            ($active ? ' AND hss.`active` = 1' : ' ').'
            ORDER BY hss.position'
        );

        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'ps_imageslider_home/images/'.$slide['image']);
            $slide['url'] = $this->updateUrl($slide['url']);
            $slide['desc_pos'] = $this->getDescPosName($slide['desc_pos']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'homebanner_slides hss ON (hs.id_homebanner_slides = hss.id_homebanner_slides)
            LEFT JOIN '._DB_PREFIX_.'homebanner_slides_lang hssl ON (hss.id_homebanner_slides = hssl.id_homebanner_slides)
            WHERE hs.`id_homebanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_homebanner_slides` as id_slide
                FROM `'._DB_PREFIX_.'homebanner` hs
                WHERE hs.`id_homebanner_slides` = '.(int)$id_slide;
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);

        return ($row);
    }

    public function renderList($slot = null)
    {
        $slides = $this->getSlides(null, $slot);
        foreach ($slides as $key => $slide) {
            $slides[$key]['status'] = $this->displayStatus($slide['id_slide'], $slide['active']);
            $associated_shop_ids = Ps_HomeSlide_Home::getAssociatedIdsShop((int)$slide['id_slide']);
        ...
        $this->context->smarty->assign(
            array(
                ...
                'slot_id' => $slot,
                'slot_name' => $this->getSlotNameById($slot)
            )
        );
    ...
    public function renderAddForm()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => $this->getSlotNameById(Tools::getValue('slot')),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                    ...
                    array(
                        'type' => 'select',
                        'label' => $this->l('Description\'s position'),
                        'name' => 'desc_pos',
                        'required' => true,
                        'options' => array(
                            'query' => array(
                                array('id'=> self::DESC_TOP, 'name' => $this->l('Top')),
                                array('id'=> self::DESC_MID, 'name' => $this->l('Middle')),
                                array('id'=> self::DESC_BOT, 'name' => $this->l('Bottom'))
                            ),
                            'id' => 'id',
                            'name' => 'name'
                        )
    ...
    public function getAddFieldsValues()
    {
        ...
        $fields['desc_pos'] = Tools::getValue('desc_pos', $slide->desc_pos);
        $fields['slot'] = Tools::getValue('slot', $slide->slot);
