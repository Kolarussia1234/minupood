...
include_once('ps_imageslider_mod.php');

$home_slider = new Ps_ImageSlider_Mod();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'homeslider_mod_slides` SET `position` = '.(int)$position.'
			WHERE `id_homeslider_mod_slides` = '.(int)$id_slide
		);
