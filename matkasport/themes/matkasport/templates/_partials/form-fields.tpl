...
{else}
  <div class="form-group row align-items-center {if !empty($field.errors)}has-error{/if} {if $field.name == "password" || $field.name == "new_password"}hide_password{/if} {if $field.name == "siret"}hide_id_number{/if}" 
    {if $field.name == "id_gender" || $field.name == "company"}
      style="display:none;"
    {/if}
  >
    <label class="col-md-2 col-form-label{if $field.required} required{/if}">
      {if $field.name == 'vp_id_code'}
        {l s='ID code' d='Shop.Forms.Labels'}
      ...
      {else}
        ...
        {block name='form_field_item_other'}
          <input
            ...
            {if $field.required}required{/if}
            {if $field.name == 'vp_id_code'} readonly disabled{/if}
          >
          {if isset($field.availableValues.comment)}
            <span class="form-control-comment">
              {$field.availableValues.comment}
    ...
    <div class="col-md-2 form-control-comment">
      {block name='form_field_comment'}
        {if (!$field.required && !in_array($field.type, ['radio-buttons', 'checkbox']) && $field.name != 'vp_id_code')}
