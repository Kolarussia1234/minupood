...
<div id="js-product-list">
    <div class="products row {if $iqitTheme.pl_default_view == 'grid'}products-grid{else}products-grid grid-x5{/if}">
        {foreach from=$listing.products item="product"}
            {block name='product_miniature'}
                ...
                {else}
                    ...
                    {include file='catalog/_partials/miniatures/product-x5.tpl' product=$product}
                {/if}
            {/block}
        {/foreach}
    </div>
